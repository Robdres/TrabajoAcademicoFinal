...
database = pd.read_csv( 'data/global_cancer_patients_2015_2024.csv' )

attributes = database["Target_Severity_Score"].sample(4000)
...
def custom_message(title, message, image_path):
    popup = tk.Toplevel()
    popup.title(title)
    ...
    label_msg = tk.Label(popup, text=message)
...
def on_predict():
    try:
        ...
        x = attributes.to_list()


        plot1 = fig.add_subplot(111)
        counts, bins, patches=plot1.hist(x, bins=20, color='skyblue', edgecolor='black')
        ...
        bin_index = np.digitize(resultado, bins) - 1
        ...
        fig.savefig("./data/.res.png")
        custom_message ("Predicción de Severidad", f"Severidad estimada: {resultado[0]:.2f}","./data/.res.png")
